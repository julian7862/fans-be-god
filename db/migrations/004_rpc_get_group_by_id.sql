-- 004_rpc_get_group_by_id.sql
-- Single query RPC to get group detail with member count (verifies membership)

CREATE OR REPLACE FUNCTION public.get_group_by_id(p_group_id UUID, p_user_id UUID)
RETURNS TABLE (
  id UUID,
  name TEXT,
  description TEXT,
